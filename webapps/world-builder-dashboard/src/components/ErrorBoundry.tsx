...
const ErrorBoundary = () => {
  ...
  if (isNotAuthorized && error) {
    error.message =
      "It seems you are not authorized, please logout and try again";
  }
      ...
      <Stack align="center" gap="xl">
        <Box>
          <Text size="sm" ta="center" c="gray">
            {error?.message ||
              error?.statusText ||
              t("We are working on a fix")}
          </Text>
          {isNotAuthorized && (
            <Text size="sm" c="gray" ta="center">
              {t(
                "Try login again and refreshing your browser before contacting support",
              )}
            </Text>
          )}
        </Box>
