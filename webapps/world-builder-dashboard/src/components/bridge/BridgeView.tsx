...
const BridgeView: React.FC = () => {
    ...
    const { L2Provider, connectedAccount, tokenAddress, L2_RPC, selectedL3Network, setSelectedL3Network } = useBlockchainContext();
    const {data: l2Balance} = useERC20Balance({tokenAddress, account: connectedAccount, rpc: L2_RPC});
    const {data: l3NativeBalance} = useNativeBalance({account: connectedAccount, rpc: selectedL3Network.chainInfo.rpcs[0]});
    ...
    return (
        <div style={{display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '100px'}}>
            ...
            <div className={styles.container}>
              ...
              <ValueToBridge title={direction === 'DEPOSIT' ? 'Deposit' : 'Withdraw'} symbol={L3_NATIVE_TOKEN_SYMBOL} value={value} setValue={setValue} balance={direction === 'DEPOSIT' ? (l2Balance ?? '0') : (l3NativeBalance ?? '0')} rate={g7tUsdRate.data ?? 0}/>
              <TransactionSummary direction={direction} gasBalance={Number((direction === "DEPOSIT" ? l2NativeBalance : l3NativeBalance) ?? 0)} address={connectedAccount ?? '0x'} transferTime={direction === 'DEPOSIT' ? '< min' : '~15 min'} fee={Number(estimatedFee.data ?? 0)} value={Number(value)} ethRate={ethUsdRate ?? 0} tokenSymbol={L3_NATIVE_TOKEN_SYMBOL} tokenRate={g7tUsdRate.data ?? 0} />
