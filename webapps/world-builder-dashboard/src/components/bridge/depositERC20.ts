...
const  generateRandomNumber = () =>  {
    ...
    const randomFractionalDigits = Array.from({ length: fractionalPart.length - 2 }, () => Math.floor(Math.random() * 10)).join('');
...
export const sendDepositTransaction = async (
    ...
    const txRequest = await ERC20InboxContract.populateTransaction.depositERC20(ethAmount, {gasLimit: gasEstimate});
