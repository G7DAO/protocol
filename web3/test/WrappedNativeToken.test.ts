    ...
    for (const { Token } of TOKENS) {
        ...
        describe(Token, function () {
            ...
            it('should not deploy with value', async function () {
                await expect(
                    ethers.deployContract(Token, [name, symbol], { value: initialSupply })
            ...
            describe('deposit', function () {
                beforeEach(async function () {
                    this.deposit = (from: HardhatEthersSigner, value: bigint) =>
                        this.token.connect(from).deposit({ value });
                });
                ...
                it('should deposit', async function () {
                    ...
                    const value = 1n;
                    await expect(this.deposit(this.holder, value))
                        .to.emit(this.token, 'Deposit')
                        .withArgs(this.holder.address, value);
                    expect(await this.token.balanceOf(this.holder.address)).to.equal(previousBalance + value);
                });

                describe('withdraw', function () {
                    beforeEach(function () {
                        this.withdraw = (from: HardhatEthersSigner, value: bigint) =>
                            this.token.connect(from).withdraw(value);
                    });
                    ...
                    it('should withdraw', async function () {
                        ...
                        await expect(this.deposit(this.holder, value))
                            .to.emit(this.token, 'Deposit')
                            .withArgs(this.holder.address, value);
                        await expect(this.withdraw(this.holder, value))
