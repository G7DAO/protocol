...
describe('TokenFaucet', function () {
    ...
    it('Should NOT set faucet amount if not owner', async function () {
        await expect(tokenFaucet.connect(addr1).setFaucetAmount(faucetAmount * 2)).to.be.revertedWithCustomError(
            tokenFaucet,
    ...
    it('Should NOT set faucet time interval if not owner', async function () {
        await expect(tokenFaucet.connect(addr1).setFaucetTimeInterval(timeInterval * 2)).to.be.revertedWithCustomError(
            tokenFaucet,
            'OwnableUnauthorizedAccount'
        );
    });
    ...
    it('Should NOT set token address if not owner', async function () {
        await expect(tokenFaucet.connect(addr1).setTokenAddress(tokenAddress)).to.be.revertedWithCustomError(
            tokenFaucet,
            'OwnableUnauthorizedAccount'
        );
    });
    ...
    it('Should allow claiming tokens', async function () {
        ...
        await expect(claimCallPromise)
            .to.emit(token, 'Transfer')
            .withArgs(tokenFaucetAddress, owner.address, faucetAmount);
    ...
    it('Should NOT allow claiming tokens if time interval not passed', async function () {
        ...
        await expect(tokenFaucet.claim()).to.be.revertedWithCustomError(
            tokenFaucet,
    ...
    it('Should allow claiming tokens if time interval passed', async function () {
        const tokenFaucetAddress = await tokenFaucet.getAddress();
        await token.transfer(tokenFaucetAddress, initialSupply);
        await expect(tokenFaucet.claim())
            .to.emit(token, 'Transfer')
            .withArgs(tokenFaucetAddress, owner.address, faucetAmount);
    ...
    it('Should transfer ownership', async function () {
        await expect(tokenFaucet.transferOwnership(addr1.address))
            .to.emit(tokenFaucet, 'OwnershipTransferred')
