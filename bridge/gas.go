...
func SetTeleporterGasParams(teleportParams *TeleportParams, teleporterAddress common.Address, l1Client *ethclient.Client, l2Client *ethclient.Client, l3Client *ethclient.Client, key *keystore.Key, teleportationType TeleportationType) (*TeleportParams, error) {
	...
	l1l2TokenBridgeGasLimit, l1l2TokenBridgeMaxSubmissionCost, l1l2TokenBridgeErr := GetL1l2TokenBridgeGasParams(l1Client, key, teleportParams, &teleportParams.L1l2Router, teleporterAddress, l2FowarderAddress)
...
func GetL1l2TokenBridgeGasParams(client *ethclient.Client, key *keystore.Key, teleportParams *TeleportParams, l1l2RouterAddress *common.Address, teleporterAddress common.Address, l2ForwarderAddress common.Address) (uint64, *big.Int, error) {
	baseFee, baseFeeErr := client.SuggestGasPrice(context.Background())
	if baseFeeErr != nil {
		return uint64(0), nil, baseFeeErr
	}
	...
	outboundCalldata, outboundCalldataErr := router.GetOutboundCalldata(&bind.CallOpts{}, teleportParams.L1Token, teleporterAddress, l2ForwarderAddress, teleportParams.Amount, []byte{})
	...
	estimateRetryableTicketCalldata, estimateRetryableTicketCalldataErr := nodeInterfaceAbi.Pack("estimateRetryableTicket", teleportParams.L1l2Router, big.NewInt(0), teleportParams.L2l3RouterOrInbox, big.NewInt(0), teleportParams.L2l3RouterOrInbox, teleportParams.L1l2Router, []byte{})
	if estimateRetryableTicketCalldataErr != nil {
		return uint64(0), nil, estimateRetryableTicketCalldataErr
	}
	...
	estimateRetryableTicketCallMsg := ethereum.CallMsg{
		...
		Value: nil,
		Data:  estimateRetryableTicketCalldata,
	}

	// TODO: Fix l1l2TokenBridgeGasLimit to use it instead of DEFAULT_GAS_LIMIT
	_, l1l2TokenBridgeGasLimitErr := client.EstimateGas(context.Background(), estimateRetryableTicketCallMsg)
	if l1l2TokenBridgeGasLimitErr != nil {
		return uint64(0), nil, l1l2TokenBridgeGasLimitErr
	}

	return DEFAULT_GAS_LIMIT, l1l2TokenBridgeMaxSubmissionCost, nil
}

// TODO: For now it's only working to bridge custom fee tokens (using Inbox contract instead of L2L3Router)
func GetL2L3TokenBridgeGasParams(client *ethclient.Client, key *keystore.Key, teleportParams *TeleportParams, l2ForwarderAddress common.Address) (uint64, *big.Int, error) {
	...
	estimateRetryableTicketCalldata, estimateRetryableTicketCalldataErr := nodeInterfaceAbi.Pack("estimateRetryableTicket", l2ForwarderAddress, senderDeposit, teleportParams.To, teleportParams.Amount, teleportParams.To, teleportParams.To, teleportParams.L3CallData)
	if estimateRetryableTicketCalldataErr != nil {
		return uint64(0), nil, estimateRetryableTicketCalldataErr
	}
	...
	_, l2l3TokenBridgeGasLimitErr := client.EstimateGas(context.Background(), estimateRetryableTicketCallMsg)
	if l2l3TokenBridgeGasLimitErr != nil {
		return uint64(0), nil, l2l3TokenBridgeGasLimitErr
	}

	return DEFAULT_GAS_LIMIT, l2l3TokenBridgeMaxSubmissionCost, nil
}
...
func GetL1l2FeeTokenBridgeGasParams(client *ethclient.Client, key *keystore.Key, teleportParams *TeleportParams, l1l2RouterAddress *common.Address, teleporterAddress common.Address, l2ForwarderAddress common.Address) (uint64, *big.Int, error) {
	...
	// TODO: Fix l1l2FeeTokenBridgeGasLimit to use it instead of DEFAULT_GAS_LIMIT
	_, l1l2FeeTokenBridgeGasLimitErr := client.EstimateGas(context.Background(), estimateRetryableTicketCallMsg)
	if l1l2FeeTokenBridgeGasLimitErr != nil {
		return uint64(0), nil, l1l2FeeTokenBridgeGasLimitErr
	}

	return DEFAULT_GAS_LIMIT, l1l2FeeTokenBridgeMaxSubmissionCost, nil
}
